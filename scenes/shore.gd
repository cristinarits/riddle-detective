...
func _on_level_spawn(destination_tag: String):
	...
	for door in doors:
		if door.door_tag == destination_tag: # fixed this line!
			target_door = door
			break
	
	if target_door:
		var player = get_node("player") # ensure path is correct
		...
		for door in doors:
			print("Available door_tag: ", door.door_tag) # debugging helper
